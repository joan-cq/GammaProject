        ...
        try {
            ...
            if ("PROFESOR".equals(user.getRol())) {
                ...
                if (profesor != null && profesor.getCurso() != null) {
                    ...
                    response.put("nombreCurso", profesor.getCurso().getNombre());
                }
            }
            logService.addLog(LogService.LOGIN, "AUTH", "Inicio de sesión exitoso (rol: " + user.getRol() + ")", dni);
            return ResponseEntity.ok(response);
        } catch (BadCredentialsException e) {
            logService.addLog(LogService.ERROR, "AUTH", "Intento de inicio de sesión fallido para el DNI: " + dni, dni);
