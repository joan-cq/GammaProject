...
function ComponenteProNotas() {
    ...
    const exportarPDF = async () => {
        if (!bimestreSeleccionado || !gradoSeleccionado) {
            Swal.fire("Atención", "Selecciona un bimestre y un grado antes de exportar.", "info");
            return;
        }
        const curso = auth.usuario?.codigoCurso;
        try {
            const response = await apiClient.get(
                `/notas/exportar?codigoGrado=${gradoSeleccionado}&idBimestre=${bimestreSeleccionado}&codigoCurso=${curso}`,
                { responseType: "blob" }
            );
            const url = window.URL.createObjectURL(new Blob([response.data], { type: "application/pdf" }));
            const link = document.createElement("a");
            link.href = url;
            link.setAttribute("download", `notas_${gradoSeleccionado}_${curso}_bim${bimestreSeleccionado}.pdf`);
            ...
            Swal.fire("Error", "No se pudo exportar el PDF de notas.", "error");
        }
    };
            ...
            <div className="container">
                ...
                <div className="row mb-3">
                    ...
                    <div className="col d-flex align-items-end gap-2">
                        ...
                        <button className="btn btn-outline-light" onClick={exportarPDF}>Exportar PDF</button>
                    </div>
                </div>
